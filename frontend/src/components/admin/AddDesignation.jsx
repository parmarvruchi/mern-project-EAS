...
const AddDesignation = () => {
  ...
  const fetchDesignations = async (deptId = "") => {
    const res = await axios.get(
      `http://https://mern-project-eas.onrender.com/api/designations?departmentId=${deptId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(
          `http://https://mern-project-eas.onrender.com/api/designations/${editingId}`,
          { name, department },
          { headers: { Authorization: `Bearer ${token}` } },
        );
        ...
        await axios.post(
          "http://https://mern-project-eas.onrender.com/api/designations",
          { name, department },
          { headers: { Authorization: `Bearer ${token}` } },
        );
  ...
  const handleDelete = async (id) => {
    try {
      if (window.confirm("Are you sure?")) {
        await axios.delete(`http://https://mern-project-eas.onrender.com/api/designations/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
