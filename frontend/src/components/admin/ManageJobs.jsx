...
const ManageJobs = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const res = await axios.get("http://https://mern-project-eas.onrender.com/api/hr/jobs", {
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://https://mern-project-eas.onrender.com/api/hr/jobs/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchJobs();
    } catch (error) {
